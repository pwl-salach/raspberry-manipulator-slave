...
class Manipulator:
    ...
    def __init__(self) -> None:
        ...
        self.base_joint = Axis(length=20, rotation=120, pivot=Pivot.Y, anchor=self.rotation_joint)
        self.elbow_joint = Axis(length=15, rotation=75, pivot=Pivot.Y, anchor=self.base_joint)
        self.wrist_joint = Axis(length=8, rotation=100, pivot=Pivot.Y, anchor=self.elbow_joint)
        self.arm_structure = [
            self.rotation_joint,
            self.base_joint,
            self.elbow_joint,
    ...
    def move_effector(self, dx: float, dy: float, dz: float) -> bool:
        ...
        if not possible_settinges:
            ...
            self.elbow_joint.rotation = 180 - possible_settinges[2]
    ...
    def calculate_potential_joints_settinges(self, target: Point) -> List[float]:
        ...
        f = A - h3 * cos(alpha2)
        e = z - h3 * sin(alpha2)
        alpha3 = acos((e**2 + f**2 - h1**2 - h2 ** 2) / (h1 * h2))
        gamma = acos((e**2 + f**2 - h1**2 + h2 ** 2) / (2 * h1 * sqrt(e**2 * f**2)))
        beta =  acos(f / sqrt(e**2 + f**2))
        alpha4 =  beta + gamma
        alpha5 = alpha4 - alpha2 - alpha3
        return [alpha1, alpha4, alpha3, alpha5]
